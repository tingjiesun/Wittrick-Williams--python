...
class Element:
    ...
    - EI: 弯曲刚度 EI
    - mass: 线密度 m（单位长度质量）
    - CosA, SinA: 单元方向余弦与正弦（构建全局坐标）
    - GlbDOF: 6 个全局自由度编号（约束用 0）
    """
    Len: float
    EA: float
    EI: float
...
if __name__ == "__main__":
    ...
    L = 1.0
    #两端简支，没有竖向，横向位移分量
    elem1 = Element(Len=3*L, EA=E * A, EI=E * I, mass=rho * A, CosA=1.0, SinA=0.0, GlbDOF=[0, 0, 0, 1, 0, 2])
